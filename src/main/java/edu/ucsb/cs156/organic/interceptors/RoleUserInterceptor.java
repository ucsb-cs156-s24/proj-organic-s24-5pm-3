...
public class RoleUserInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
            throws Exception {
        // Update user's security context on server each time the user makes HTTP
        // request to the backend
        // If user has admin status in database we will keep ROLE_ADMIN in security
        // context
        // Otherwise interceptor will remove ROLE_ADMIN before the incoming request is
        // processed by backend API
        ...
        if (authentication instanceof OAuth2AuthenticationToken) {
            ...
            if (optionalUser.isPresent()) {
                ...
                Authentication newAuth = new OAuth2AuthenticationToken(oAuthUser, newAuthorities,
